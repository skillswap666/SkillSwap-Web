...
export function Dashboard() {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      try {
        setLoading(true);
        // For now, we'll use mock data since we don't have workshop/transaction APIs yet
        // In a real app, you'd fetch from your backend APIs here
        setWorkshops([]);
        setTransactions([]);
      } catch (error) {
            ...
            <Card>
              <CardContent className="p-6">
                <div className="text-center">
                  <Avatar className="w-20 h-20 mx-auto mb-4 border shadow-sm">
                    {user?.profilePicture ? (
                      <AvatarImage
